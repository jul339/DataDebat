...
class EmbeddingGenerator:
    ...
    MODELS = {
        "camembert": "dangvantuan/sentence-camembert-base",  # 768 dims, bon équilibre
        "multilingual": "intfloat/multilingual-e5-large",     # 1024 dims, très performant
        "solon": "OrdalieTech/Solon-embeddings-large-0.1",    # Juridique/administratif FR
    }
    
    def __init__(self, model_name: str = "dangvantuan/sentence-camembert-base",
    ...
    def generate_batch(self, texts: List[str], batch_size: int = 32, 
    ...
    def enrich_documents_with_embeddings(self, index_name: str, 
                                          text_field: str = "texte",
                                          batch_size: int = 100,
                                          query: dict = None) -> Tuple[int, List]:
        """
        ...
        search_query = {
            "bool": {
                "must_not": {"exists": {"field": "texte_embedding"}}
            }
        }
        if query:
        ...
        for i in tqdm(range(0, len(docs), batch_size), desc="Génération embeddings"):
            ...
            if valid_texts:
                embeddings = self.generate_batch(valid_texts, batch_size=len(valid_texts))
    ...
    def semantic_search(self, query: str, index_name: str, 
                        k: int = 10, num_candidates: int = 100,
    ...
    def get_yearly_centroids(self, index_name: str, 
                              year_field: str = "annee",
                              topic_filter: dict = None) -> Dict[int, np.ndarray]:
        """
        ...
        agg_query = {
            ...
            "aggs": {
                "years": {
                    "terms": {"field": year_field, "size": 100}
                }
            }
        }
    ...
    def find_similar_across_time(self, reference_text: str, index_name: str,
                                   year_field: str = "annee",
                                   k_per_year: int = 5) -> Dict[int, List[Dict]]:
        """
    ...
    def compare_groups(self, index_name: str, 
                        group_field: str,
                        groups: List[str],
                        topic_filter: dict = None) -> Dict[str, Dict]:
        """
